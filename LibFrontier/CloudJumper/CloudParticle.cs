...
public class CloudParticle {
    ...
    public void Update(Random random) {
        ...
        Func<byte, byte> transform = i => (byte)Math.Max(0, i - random.Next(0, 2));
        symbol = symbol with { Foreground = ABGR.RGBA(transform(r), transform(g), transform(b), transform(a)) };
    }
    ...
    public static void CreateClouds(int effectMinY, int effectMaxY, List<CloudParticle> clouds, Random random) {
        ...
        var cloudParticle = new Tile(ABGR.RGB((byte)(204 + random.Next(0, 51)), 0, (byte)(204 + random.Next(0, 51))), ABGR.Transparent, GetRandomChar());
        ...
        while (random.NextDouble() < 0.9) {
            ...
            cloudParticle = new Tile(ABGR.RGB((byte)(204 + random.Next(0, 51)), 0, (byte)(225 + random.Next(0, 25))), ABGR.Transparent, GetRandomChar());
            clouds.Add(new CloudParticle(cloudPoint, cloudParticle));
            for (int y = 1; y < random.Next(2, 5); y++) {
                var verticalPoint = (cloudPoint.X - 0, cloudPoint.Y - y);
                cloudParticle = new Tile(ABGR.RGB((byte)(225 + random.Next(0, 25)), (byte)(153 + random.Next(102)), (byte)(225 + random.Next(0, 25))), ABGR.Transparent, GetRandomChar());
